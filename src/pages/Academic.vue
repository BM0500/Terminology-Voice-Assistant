      ...
      <div class="container mx-auto px-4 py-8">
        ...
        <section class="mb-10 text-center">
          <h2 class="text-[clamp(1.8rem,5vw,3rem)] font-bold text-neutral mb-4">
            <i class="fa-solid fa-file-text text-blue-600 mr-4"></i>
            学术论文术语
          </h2>
          <p class="text-neutral-dark max-w-2xl mx-auto text-lg">
            掌握学术研究、论文写作、统计分析等领域的专业术语
          </p>
        </section>
...
// 从本地存储加载数据
onMounted(() => {
  const savedFavorites = localStorage.getItem('academic-favorites')
  const savedLearned = localStorage.getItem('academic-learned')
...
const toggleFavorite = (termId: number) => {
  ...
  localStorage.setItem('academic-favorites', JSON.stringify(favorites.value))
}
...
const markAsLearned = (termId: number) => {
  if (!learnedTerms.value.includes(termId)) {
    ...
    localStorage.setItem('academic-learned', JSON.stringify(learnedTerms.value))
  }
}
</script>
